...
def colsetup(columns):
    ...
    for i in columns:
        if i in percentagecols:
            ...
            cols.append(
                {"id": i, "name": i, "type": "text", "presentation": "dropdown"}
            )
        else:
...
@app.callback(
    Output("walletoverview_upload", "data"),
...
@app.callback(
    Output("walletoverview_global", "data"),
...
@app.callback(
    Output("walletoverview_tbl", "data"),
...
def setupaddtable(trigger, data):
    ...
    dropdowns = {
        "chainid": {
            "options": [{"label": i["label"], "value": i["chain_id"]} for i in r]
        },
    }
    if data is None:
        return DataTable(
